...
where i.type='Usual' and (it.count < L.every_day OR isnull(it.count))  and ya.status = 'Active'
order by it.count limit 1";

const GET_INTERFACE_FOR_GOOGLE = "SELECT  `interf`.`id`, `interf`.`interface`,`interf`.`port`, `interf`.`type`,`interf`.`proxy_login`, `interf`.`proxy_password`
